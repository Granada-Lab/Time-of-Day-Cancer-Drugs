...
%Function creates butterfly charts to evaluate the extend of the difference in responses between the cancer cell line and MCF10A 
...
for a = 1:2 %loop a channel
    ...
    for i = 1:ii %loop i input
        ...
        %average data
        % by drug
        meandrug(:,i) = mean((alldata-1),2,'omitnan');
        stddrug(:,i) = std((alldata-1),[],2,'omitnan');
        % by cellline
        meancell(:,i) = rmmissing(mean((alldata-1),1,'omitnan'));
        stdcell(:,i) = rmmissing(std((alldata-1),[],1,'omitnan'));
    ...
    for k = 1:2 %loop k different plots (mean drug or celllines)
        ...
        xlabel('Mean foldchange in relative cell numbers');
        ...
        %save figure
        filename = append('ToD_plots/',channel{a},'/butterfly_max_min_benefit/butterly_',name,'_mean_',channel{a});
        saveas(fig, [filename '.svg']);

        valuestoclear1 = {'data';'drug';'sortIdx';'x';'y1';'y2';'err'};
        clear(valuestoclear1{:});

    end %loop k different plots

    valuestoclear2 = {'alldata';'meandrug';'stddrug';'meancell';'stdcell'};
