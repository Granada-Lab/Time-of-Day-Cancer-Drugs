function clustering_ranking_ToDMR_values(celllines_clustergram,channel,drugs,color)
...
drugs(end,:) = []; %exclude Olaparib (only 2 cell lines tested)
ybarmax_d = 8.6;
ybarmax_c = 10.6;
...
for a = 1:2 %loop a channel
    ...
    inputfile = 'ToD_values_splinesmooth_benefit.xlsx';
    sheet = append('ToDMR_spline_',channel{a});

    [data] = readmatrix(inputfile,'Sheet',sheet);
    data(9,:) = []; %exclude HCC1937_2
    data(:,[1,end]) = []; %exclude first row (all NaN) & Olaparib (only two celllines)
    ...
    celllinesorted = celllines_clustergram(sortIdx_c,:);
    ...
    set(cgo,'ColumnLabelsRotate',90,'RowLabelsRotate',0,'Displayrange',0.5);
