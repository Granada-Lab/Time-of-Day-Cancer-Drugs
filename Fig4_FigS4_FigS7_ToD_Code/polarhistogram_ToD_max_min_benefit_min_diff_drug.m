function polarhistogram_ToD_max_min_benefit_min_diff_drug(file,excelsheets_times,channel,drugs)
...
for a = 1:2 %loop metric
    ...
    for d = 1:dd %loop drug
        ...
        sgtitle(drugs{d},'FontSize',14,'FontName','Helvetica Neue','FontWeight','bold');
        ...
        for i = 1:ii %loop input
            ...
            alldata(:,end-3) = []; %remove HCC1937_2
        ...
        for k = 1:2 %create two polarhistograms side-by-side
            ...
            for j = 1:3
                ...
                n = size(rmmissing(data(j,:),2));
                titletext = append(titles{k},' n=',num2str(n(:,2)));
        ...
        end %loop k polarhistograms

        %save figure
        filename = append('ToD_plots/',channel{a},'/polarhistogram_max_min_times/polarhistogram_',drugs{d},'_',channel{a});
